...
inline __device__ void print_float3(const float3 x, const char* s, const int32_t tid) {
    printf("(tid = %d) %s: %f %f %f\n", tid, s, x.x, x.y, x.z);
}

inline __device__ void print_int3(const int3 x, const char* s, const int32_t tid) {
    printf("(tid = %d) %s: %d %d %d\n", tid, s, x.x, x.y, x.z);
}
    ...
    float *terminate_planes)
{
    ...
    for (int32_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < n_rays; tid += blockDim.x * gridDim.x)
    {
        ...
        for (int32_t i = base_t_sorted; i < base_t_sorted + n_grids * 2 - 1; i++) {
            ...
            // printf(
            //     "[traverse init A] (tid = %d), delta=(%f, %f, %f), step_index=(%d, %d, %d)\n",
            //     tid, delta.x, delta.y, delta.z, step_index.x, step_index.y, step_index.z
            // );
            ...
            while (traverse_steps_limit <= 0 || n_samples < traverse_steps_limit) {
                ...
                // A temporary solution. It is not meant to alter nerfacc significantly. 
                if (
                    (current_index.x < 0) || (current_index.x >= resolution.x) ||
                    (current_index.y < 0) || (current_index.y >= resolution.y) ||
                    (current_index.z < 0) || (current_index.z >= resolution.z)
                ) {
                    break;
                }
            }
