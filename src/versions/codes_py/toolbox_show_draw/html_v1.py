                        ...
                        with p():
                            ...
                            if not(txt is None):
                                if type(txt) in [list, tuple]:
                                    for t in txt:
                                        p(t)
                                elif type(txt) in [str]:
                                    p(txt)
                                else:
                                    raise ValueError(type(txt))
...
def storeImagesAndReturnLinks(str_store, str_link, strs_imgs, imgsContents):
    ...
    for i in range(len(strs_imgs)):
        assert type(imgsContents[i]) == np.ndarray, (type(imgsContents[i]), str(strs_imgs[i]))
        if len(imgsContents[i].shape) in [2, 3]:  # It is png
            if len(imgsContents[i].shape) == 2:
                assert imgsContents[i].shape[0] > 5, (imgsContents[i].shape, strs_imgs[i])
                assert imgsContents[i].shape[1] > 5, (imgsContents[i].shape, strs_imgs[i])
            if 'float32' in str(imgsContents[i].dtype):
                ...
                raise ValueError("Let's disable this branch. %s %s" % (imgsContents[i].dtype, strs_imgs[i]))
            ...
            raise ValueError('Shape of imgsContents[i] is wierd %s (%s)' % (str(imgsContents[i].shape), str(strs_imgs[i])))
...
class HTMLStepperAbstract(object):
    ...
    def step2(self, summary0=None, txt0=None, brInds=(0, ), headerMessage=None, subMessage=None):
        ...
        if self.htmlStepCount >= self.singleHtmlSteps:  # reset
            ...
            os.makedirs(self.logDir + "html_" + self.htmlName + "/", exist_ok=True)
