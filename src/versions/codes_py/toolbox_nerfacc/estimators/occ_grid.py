...
class OccGridEstimator(AbstractEstimator):
    ...
    def sampling(
        ...
        **kwargs,  # debugging purpose
        ...
        batch_head_index = kwargs.get("batch_head_index")
    ...
    def _update(
        ...
        for lvl, indices in enumerate(lvl_indices):
            ...
            """ modification: chunkenize the prediction to save GPU memories
            occ = occ_eval_fn(x).squeeze(-1)
            """
            occ_list = []
            chunk = 32768
            for i in range(0, x.shape[0], chunk):
                occ_list.append(occ_eval_fn(x[i:i + chunk]).squeeze(-1))
            occ = torch.cat(occ_list, 0)
            del occ_list, chunk
            """"""
