# Implemented referencing https://github.com/LinjieLyu/NRTF/blob/main/src/model.py
...
class PRTNetwork1(nn.Module):
    ...
    def forward(self,pos,dout,din,mode="original"):
        if mode == "original":
            N=pos.shape[0]
            M=din.shape[0]  
            ...
            for k, x in [("pos", pos), ("dout", dout), ("din", din)]:
                assert len(x.shape) == 2, ("%s shape is %s" % (k, x.shape))
                assert x.shape[0] == pos.shape[0], ("%s shape is %s, while pos is %s" % (k, x.shape, pos.shape))
            all_input = torch.cat([pos, dout, din], dim=-1).view(pos.shape[0], -1)
        else:         
