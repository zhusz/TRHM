# Implemented referencing https://github.com/LinjieLyu/NRTF/blob/main/src/hash_encoding.py
...
def get_voxel_vertices(xyz, bounding_box, resolution, log2_hashmap_size):
    ...
    voxel_indices = bottom_left_idx.unsqueeze(1).long() + BOX_OFFSETS.to(bottom_left_idx.device)
...
def get_hash_table(xyz,bounding_box, resolution):
    ...
    HASH_OFFSETS =torch.tensor([[-1,1,-resolution,resolution,-resolution**2,resolution**2]],device=xyz.device)
    for j in range(2):   
...
class HashEmbedder(nn.Module):
    def __init__(self, bounding_box=[-1.0,1.0], n_levels=16, n_features_per_level=2,\
        ...
        self.bounding_box_np = [x.detach().float().cpu().numpy() for x in bounding_box]
        ...
        for i in range(n_levels):
            ...
            hash_table_np=get_hash_table(vertices,bounding_box, resolution).long().detach().cpu().numpy()
            
            embedding_layer=nn.Embedding(hash_table_np.shape[0], self.n_features_per_level,sparse=sparse)
            self.embedding_layers.append(embedding_layer)
            self.hash_tables_np.append(hash_table_np)
    ...
    def forward(self, x):
        ...
        x=x.view(-1,3)
        x_embedded_all = []
        bounding_box = [torch.from_numpy(y).to(x.device) for y in self.bounding_box_np]
        for i in range(self.n_levels):
            ...
            hashed_voxel_indices_=torch.searchsorted(
                torch.from_numpy(self.hash_tables_np[i]).to(x.device), hashed_voxel_indices)
