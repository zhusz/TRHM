...
template <typename scalar_t>
__global__ void kernel_march_rays_train_raysInfo(
    const scalar_t * __restrict__ rays_o,
    const scalar_t * __restrict__ rays_d,  
    const scalar_t * __restrict__ raysInfo,  // the added one
    const uint8_t * __restrict__ grid,
    const float bound,
    const float dt_gamma, const uint32_t max_steps,
    const uint32_t N, const uint32_t C, const uint32_t H, const uint32_t M,
    ...
    const scalar_t* __restrict__ nears, 
    const scalar_t* __restrict__ fars,
    scalar_t * xyzs, scalar_t * dirs,
    scalar_t * sampledRaysInfo,  // the added one
    scalar_t * deltas,
    int * rays,
    int * counter,
    const scalar_t* __restrict__ noises
    ...
    rays_o += n * 3;
    rays_d += n * 3;
    raysInfo += n * DInfo;  // the added one
    ...
    xyzs += point_index * 3;
    dirs += point_index * 3;
    sampledRaysInfo += point_index * DInfo;  // the added one
    deltas += point_index * 2;
    ...
    while (t < far && step < num_steps) {
        ...
        if (occ) {
            ...
            dirs[1] = dy;
            dirs[2] = dz;
            for (int o = 0; o < DInfo; o++) {  // the added one
                sampledRaysInfo[o] = raysInfo[o];
            }
            t += dt;
            deltas[0] = dt;
            deltas[1] = t - last_t; // used to calc depth
            ...
            xyzs += 3;
            dirs += 3;
            sampledRaysInfo += DInfo;  // correct  // the added one
            deltas += 2;
            step++;
        // else, skip a large step (basically skip a voxel grid)
        } else {
...
void march_rays_train_raysInfo(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    rays_o.scalar_type(), "march_rays_train", ([&] {
        kernel_march_rays_train_raysInfo<<<div_round_up(N, N_THREAD), N_THREAD>>>(
            rays_o.data_ptr<scalar_t>(),
            rays_d.data_ptr<scalar_t>(),
            raysInfo.data_ptr<scalar_t>(),  // the added one
            ...
            sampledRaysInfo.data_ptr<scalar_t>(),  // the added one
...
#define SQR(x) ((x) * (x))
template <typename scalar_t>
__global__ void kernel_composite_rays_train_refnerf_forward(
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs,  
    const scalar_t * __restrict__ normal_pred,  // newly added
    const scalar_t * __restrict__ normal_from_tau,  // newly added
    const scalar_t * __restrict__ deltas,
    const int * __restrict__ rays,
    const scalar_t * __restrict__ rays_d,  // newly added
    const uint32_t M, const uint32_t N, const float T_thresh, 
    scalar_t * weights_sum,
    scalar_t * depth,
    scalar_t * image,
    scalar_t * loss_tie,  // newly added
    scalar_t * loss_backface  // newly added
) {
    // parallel per ray
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= N) return;
    ...
    if (num_steps == 0 || offset + num_steps > M) {
        ...
        image[index * 3 + 1] = 0;
        image[index * 3 + 2] = 0;
        loss_tie[index] = 0;  // newly added
        loss_backface[index] = 0;  // newly added
        return;
    }
    ...
    rgbs += offset * 3;
    deltas += offset * 2;
    normal_pred += offset * 3;  // newly added
    normal_from_tau += offset * 3;  // newly added

    rays_d += index * 3;  // newly added
    ...
    scalar_t T = 1.0f;
    scalar_t r = 0, g = 0, b = 0, ws = 0, t = 0, d = 0;
    scalar_t loss_tie_accmulator = 0, loss_backface_accumulator = 0;  // newly added
    ...
    while (step < num_steps) {
        ...
        loss_tie_accmulator += weight * (
            SQR(normal_pred[0] - normal_from_tau[0]) + 
            SQR(normal_pred[1] - normal_from_tau[1]) + 
            SQR(normal_pred[2] - normal_from_tau[2])
        );  // newly added
        const scalar_t normal_pred_dot_d = (
            normal_pred[0] * rays_d[0] +
            normal_pred[1] * rays_d[1] +
            normal_pred[2] * rays_d[2]
        )  // newly added
        if normal_pred_dot_d > 0 {
            loss_backface_accumulator += weight * normal_pred_dot_d;
        }  // newly added
        ...
        rgbs += 3;
        deltas += 2;
        normal_pred += 3;  // newly added
        normal_from_tau += 3;  // newly added
    ...
    image[index * 3 + 1] = g;
    image[index * 3 + 2] = b;
    loss_tie[index] = loss_tie_accmulator;  // newly added
    loss_backface[index] = loss_backface_accumulator;  // newly added
}


void composite_rays_train_refnerf_forward(
    const at::Tensor sigmas, const at::Tensor rgbs,
    const at::Tensor normal_pred, const at::Tensor normal_from_tau,  // newly added
    const at::Tensor deltas, const at::Tensor rays,
    const at::Tensor rays_d,  // newly added
    const uint32_t M, const uint32_t N, const float T_thresh,
    at::Tensor weights_sum, at::Tensor depth, at::Tensor image,
    at::Tensor loss_tie, at::Tensor loss_backface  // newly added
) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    sigmas.scalar_type(), "composite_rays_train_refnerf_forward", ([&] {
        kernel_composite_rays_train_refnerf_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(
            sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(),
            normal_pred.data_ptr<scalar_t>(), normal_from_tau.data_ptr<scalar_t>(),  // newly added
            deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(),
            rays_d.data_ptr<scalar_t>(),  // newly added
            M, N, T_thresh,
            weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(),
            loss_tie.data_ptr<scalar_t>(), loss_backface.data_ptr<scalar_t>()  // newly added
        );
    }));
}
...
template <typename scalar_t>
__global__ void kernel_composite_rays_train_refnerf_backward(
    const scalar_t * __restrict__ grad_weights_sum,
    const scalar_t * __restrict__ grad_image,
    const scalar_t * __restrict__ grad_loss_tie,  // newly added
    const scalar_t * __restrict__ grad_loss_backface,  // newly added
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs, 
    const scalar_t * __restrict__ deltas,
    const int * __restrict__ rays,
    ...
    scalar_t * grad_sigmas,
    scalar_t * grad_rgbs,
    scalar_t * grad_normal_pred,  // newly added
    scalar_t * grad_normal_from_tau  // newly added
) {
    // parallel per ray
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= N) return;
    ...
    grad_weights_sum += index;
    grad_image += index * 3;
    grad_loss_tie += index;  // newly added
    grad_loss_backface += index;  // newly added
    weights_sum += index;
    image += index * 3;
    sigmas += offset;
    rgbs += offset * 3;
    ...
    grad_sigmas += offset;
    grad_rgbs += offset * 3;
    grad_normal_pred += offset * 3;  // newly added
    grad_normal_from_tau += offset * 3;  // newly added
    ...
    while (step < num_steps) {
        ...
        for (int c = 0; c < 3; c++) {
            grad_normal_pred[c] = weight * (
                grad_loss_tie[0] * 2 * (normal_pred[c] - normal_from_tau[c])
            );
            if (normal_pred_dot_d > 0) {
                grad_normal_pred[c] += weight * rays_d[c];
            }
            grad_normal_from_tau[c] = weight * (
                grad_loss_backface[0] * 2 * (normal_from_tau[c] - normal_pred[c])
            );
        }
        ...
        grad_sigmas++;
        grad_rgbs += 3;
        grad_normal_pred += 3;  // newly added
        grad_normal_from_tau += 3;  // newly added
...
void composite_rays_train_refnerf_backward(
    const at::Tensor grad_weights_sum, const at::Tensor grad_image,
    const at::Tensor grad_loss_tie, const at::Tensor grad_loss_backface,  // newly added
    ...
    const at::Tensor deltas, const at::Tensor rays,
    const at::Tensor rays_d,  // newly added
    const at::Tensor weights_sum, const at::Tensor image,

    const uint32_t M, const uint32_t N, const float T_thresh,

    at::Tensor grad_sigmas, at::Tensor grad_rgbs,
    at::Tensor grad_normal_pred, at::Tensor grad_normal_from_tau  // newly added
) {

    static constexpr uint32_t N_THREAD = 128;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_train_refnerf_backward", ([&] {
        kernel_composite_rays_train_refnerf_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(
            grad_weights_sum.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(),
            grad_loss_tie.data_ptr<scalar_t>(), grad_loss_backface.data_ptr<scalar_t>(),  // newly added
            ...
            deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(),
            rays_d.data_ptr<scalar_t>(),  // newly added
            weights_sum.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(),

            M, N, T_thresh,

            grad_sigmas.data_ptr<scalar_t>(), grad_rgbs.data_ptr<scalar_t>(),
            grad_normal_pred.data_ptr<scalar_t>(), grad_normal_from_tau.data_ptr<scalar_t>()  // newly added
        );
    }));
}
*/