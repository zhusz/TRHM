...
from .renderer_hint4 import NeRFRenderer
...
class NeRFNetwork(NeRFRenderer):
        ...
        **kwargs,
        ...
        for i in range(D_appearance):
            if i == 0:
                ...
                layer = nn.Linear(
                    W_bottleneck + in_channels_ide + 1 + in_channels_dir + in_channels_hint_pointlight_opacities + in_channels_hint_pointlight_ggx,
                    W_appearance
                )
            elif i in skips_appearance:
                layer = nn.Linear(
                    W_appearance + W_bottleneck + in_channels_ide + 1 + in_channels_dir + in_channels_hint_pointlight_opacities + in_channels_hint_pointlight_ggx,
                    W_appearance
                )
    ...
    def appearance(self, x, d, preAppearancePredictionDict, **kwargs):
        ...
        hints_pointlight_ggx_embedded = self.embeddingHintPointlightGGX(
            hints_pointlight_ggx
        )
        ...
        fullPredictionDict["hints_pointlight_opacities"] = hints_pointlight_opacities
