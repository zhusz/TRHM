...
from codes_py.toolbox_framework.framework_util_v4 import (
    ...
    save_network,
    save_general,
    cleaning_saved_redundent_networks,
)
...
class HdrClipRawNerfQuickLoss(nn.Module):  
    ...
    def __init__(self, eps=None, hdr_clip=None, Lp=None):
        super(HdrClipRawNerfQuickLoss, self).__init__()
        self.eps = eps if eps is not None else 1.0e-3
        assert hdr_clip is not None
        self.hdr_clip = hdr_clip  # if hdr_clip is not None else 2.5
        self.Lp = Lp  # L1 loss or L2 loss
        
    def forward(self, inputs, targets):
        targets = torch.clamp(targets, max=self.hdr_clip)
        Lp = self.Lp
        if Lp == 1:
            return ((torch.abs(inputs - targets)) / (torch.abs(inputs).detach() + self.eps)).mean()
        elif Lp == 2:
            return (((inputs - targets) ** 2) / (torch.abs(inputs).detach() + self.eps)).mean()  # actually your L2 version is different from raw-nerf's recommendation...
        else:
            raise NotImplementedError("Unknown Lp: %d" % Lp)
...
class Trainer(object):
    ...
    def _netFinetuningInitialization(self):
        ...
        if hasattr(self.config, "finetuningPDSRI"):
            ...
            for k in self.models.keys():
                if (k != "model") and (k != "modelDensityTeacher"):
                    continue
                if True:
    ...
    def metaLossPrepare(self):
        ...
        self.criterion_clippped = HdrClipRawNerfQuickLoss(hdr_clip=config.hdr_clipped_clip, Lp=config.hdrloss_clipped_lp)
        self.criterion_highlight = HdrClipRawNerfQuickLoss(hdr_clip=config.hdr_highlight_clip, Lp=config.hdrloss_highlight_lp)
    ...
    def initializeAll(self, **kwargs):
        ...
        if if_need_metaDataLoading:
            self.setupMonitor()  # monitoring is useful only under the training settings
    ...
    def saveModelSnapshot(self, **kwargs):
        ...
        if self.rank == self.trackedRank:
            ...
            # cleaning
            if type(iterCount) is int:
                cleaning_saved_redundent_networks(self.logDir, kept_iters=np.array([iterCount, 10000, 40000, 160000, 400000], dtype=np.int32))
        ...
        with torch.no_grad():
            ...
            for b in range(batchTot):
                ...
                t = models["meta"]["density_fn"](
                    ...
                    modelDensityTeacher=models.get("modelDensityTeacher"),
    ...
    def forwardNetNeRF(self, batch_thgpu, **kwargs):
        ...
        if ("lossTie" in wl.keys()) and (wl["lossTie"] > 0):
            ...
            lossTie = 0
        if ("lossTieHighlight" in wl.keys()) and (wl["lossTieHighlight"] > 0):
            lossTieHighlight = wl["lossTieHighlight"] * torch.cat([  # results_highlight["lossTieRays_fine"].mean()
                results_hs["highlight"]["lossTieRays_fine"],
                results_hs["sublight"]["lossTieRays_fine"],
            ], 0).mean()
        else:
            lossTieHighlight = 0
        ...
        if ("lossBackface" in wl.keys()) and (wl["lossBackface"] > 0):
            ...
            lossBackface = 0
        if ("lossBackfaceHighlight" in wl.keys()) and (wl["lossBackfaceHighlight"] > 0):
            lossBackfaceHighlight = wl["lossBackfaceHighlight"] * torch.cat([  # results_highlight["lossBackfaceRays_fine"].mean()
                results_hs["highlight"]["lossBackfaceRays_fine"],
                results_hs["sublight"]["lossBackfaceRays_fine"],
            ], 0).mean()
        else:
            lossBackfaceHighlight = 0
        ...
        if ("lossGeneralHdr2" in wl.keys()) and (wl["lossGeneralHdr2"] > 0):
            lossGeneralHdr2 = wl["lossGeneralHdr2"] * self.criterion_clippped(
                results["hdr2_fine"],
                batch_thgpu["hdrs_%s" % dataset],
            ).mean()
        else:
        ...
        if ("lossTie23" in wl.keys()) and (wl["lossTie23"] > 0):
            lossTie23 = wl["lossTie23"] * (results["opacity_fine"][:, None].detach() * torch.abs(
                results["normal2_fine"] - results["normal3_fine"]
            )).mean()
        else:
            lossTie23 = 0

        if ("lossTie23Highlight" in wl.keys()) and (wl["lossTie23Highlight"] > 0):
            lossTie23Highlight = wl["lossTie23Highlight"] * (
                torch.cat([
                    results_hs["highlight"]["opacity_fine"].detach(),
                    results_hs["sublight"]["opacity_fine"].detach(),
                ], 0)[:, None]
                * 
                torch.cat([
                    torch.abs(results_hs["highlight"]["normal2_fine"] - results_hs["highlight"]["normal3_fine"]),
                    torch.abs(results_hs["sublight"]["normal2_fine"] - results_hs["sublight"]["normal3_fine"]),
                ], 0)
            ).mean()
        else:
            lossTie23Highlight = 0
        ...
        batch_thgpu["lossTieHighlight"] = lossTieHighlight
        batch_thgpu["lossBackfaceHighlight"] = lossBackfaceHighlight
        batch_thgpu["lossHighlightNdotH"] = lossHighlightNdotH
        batch_thgpu["lossSuppressHdr3"] = lossSuppressHdr3
        batch_thgpu["lossTie23"] = lossTie23
        batch_thgpu["lossTie23Highlight"] = lossTie23Highlight
        batch_thgpu["loss"] = (
            lossMain + lossMask + lossTie + lossBackface +
            lossMainHighlight + lossMaskHighlight + lossTieHighlight + lossBackfaceHighlight +
