...
def getConfigFunc(P, D, S, R, **kwargs):
    ...
    config.datasetConfDict["capture7jf"] = {
        ...
        "ifPreloadHighlight": (get_trailing_number(R) in [2, 3]),
        "highlightBucket": "highlight331b",
        "batchSizeHighlight": 512 if (get_trailing_number(R) in [2, 3]) else 0,
        "batchSizeSublight": 512 if (get_trailing_number(R) in [2, 3]) else 0,
    ...
    config.hash_n_features_per_level = 8
    config.sigma_b_dim = 63
    config.sigma_n_hidden_layers = 4  # should get shortened later for the purpose of computing efficiency.
    config.color_net_hidden_layers = 6
    ...
    s = (0.06 / 10 / 10) if (caseID in [2, 3]) else 0  # V5a
    ...
    wl["lossHighlightNdotH"] = (0.1 * 2) if (caseID in [2, 3]) else 0  # V2
    wl["lossSuppressHdr3"] = 0.001 / 2 * 10
    config.wl = wl
    config.sloss = s   
    ...
    config.finetuningPDSRI_slow = {
        ...
        "D": "D15sepo" if (caseID in [2, 3]) else "D14sepo3",
        "S": "SgCap7V4",
        "R": "R%d" % get_trailing_number(R),
        "I": 160960 if (caseID in [7, 8, 10]) else 400960,
    }
    config.finetuningPDSRI_fast = copy.deepcopy(config.finetuningPDSRI_slow)
