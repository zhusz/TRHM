...
def getConfigFunc(P, D, S, R, **kwargs):
    ...
    config.datasetConfDict["capture7jf"] = {
        ...
        "ifPreloadHighlight": (caseID in highlight_case_list),
        "highlightBucket": "highlight331b",
        "batchSizeHighlight": 512 if (caseID in highlight_case_list) else 0,
        "batchSizeSublight": 512 if (caseID in highlight_case_list) else 0,
    ...
    wl = {}
    wl["lossMain"] = 1.0
    s = (0.06 / 10 / 10) if (caseID in highlight_case_list) else 0  # V2
    wl["lossMainHighlight"] = s * wl["lossMain"]

    wl["lossSuppressHdr3"] = 0.001 / 2
    config.wl = wl
    config.sloss = s
    config.ifShrinkNdotHloss = False  # actually this is not a necessary parameter, because the NDotH loss is set 0 now.
    ...
    if caseID in [7, 8, 10]:
        config.finetuningPDSRI_densityTeacher = {
            "hehe": "generally, this is not useful for all the cases of the real data"
        }
    else:
        config.finetuningPDSRI_densityTeacher = {
            "hehe": "generally, this is not useful for all the cases of the real data"
