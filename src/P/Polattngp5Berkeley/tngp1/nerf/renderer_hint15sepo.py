...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, models, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        with torch.no_grad():
            ...
            # normal2 - just for visualization purpose
            ...
            normal_2 = F.normalize(normal_2_unormalized, p=2, dim=1)
            ...
            if lgtMode == "pointLightMode":
                ...
                hint_ref_levels = []
                for l in range(self.gauL):
                    if l == config.get("gauRemoveLevel", 9999):
                        assert l > 0
                        hint_ref_levels.append(torch.zeros_like(hint_ref_levels[0]))
                        continue
                    if config.get("ablation_tag", 0) == 0:
                        gauKSize = self.gauKSizeList[l]
                        gauKSigma = self.gauKSigmaList[l]
                        gauHSum = self.gauHSum[l]
                    elif config.get("ablation_tag", 0) == 3:
                        gauKSize = self.gauKSizeList[0]
                        gauKSigma = self.gauKSigmaList[0]
                        gauHSum = self.gauHSum[0]
                    elif config.get("ablation_tag", 0) == 4:
                        gauKSize = self.gauKSizeList[-1]
                        gauKSigma = self.gauKSigmaList[-1]
                        gauHSum = self.gauHSum[-1]
                    else:
                        raise NotImplementedError("Unknown config.ablation_tag: %d" % config.get("ablation_tag", 0))
                    # if self.gauKSizeList[l] == 0:
                    if gauKSize == 0:
                ...
                for l in range(self.gauL):
                    if kwargs.get("specRoughnessID", None) is None:
                        if config.get("ablation_tag", 0) == 0:
                            ...
                            print("We do not do any ablation mode implementation when lgtMode == 'envmapMode'")
                            raise NotImplementedError("Unknown config.ablation_tag: %d" % config.get("ablation_tag", 0))
                    else:
                        raise ValueError("This branch should never reach.")
            ...
            hint_ref_opacities = model.embeddingHintPointlightOpacities(ref_out["opacities"])
        ...
        # hdr3
        tmp = model.normal3_envmap(
            torch.cat([
                hint_ref_all,
                model.embeddingXyz(ref_point),
            ], 1)
        )
        normal3_toadd = model.safe_exp(tmp, 1, 10).float()
