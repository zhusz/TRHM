...
class NeRFNetwork(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.if_self_is_model_density_teacher = kwargs["if_self_is_model_density_teacher"]
        ...
        self.normal3_alpha = tcnn.Network(
            n_input_dims=32,
            n_output_dims=1,
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
                "output_activation": "None",  # need trunc_exp in forward
                "n_neurons": 64,
                "n_hidden_layers": 1,
            }
        )
        self.normal3_hdr = tcnn.Network(
            n_input_dims=32 + in_channels_hint_pointlight_opacities,
            n_output_dims=3,
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
    ...
    def density(self, xyz, if_do_bound_clamp, if_output_only_sigma, if_compute_normal3, **kwargs):
        ...
        if config.get("needDensityFixer", False) and (not self.if_self_is_model_density_teacher):
            modelDensityTeacher = kwargs["modelDensityTeacher"]
            with torch.no_grad():
                teacher_xyz_ = xyz_embedded
                for i in range(modelDensityTeacher.D):
                    if i in modelDensityTeacher.skips:
                        teacher_xyz_ = torch.cat([xyz_embedded, teacher_xyz_], -1)
                    teacher_xyz_ = getattr(modelDensityTeacher, f"xyz_encoding_{i+1}")(teacher_xyz_)
                teacher_sigma = modelDensityTeacher.sigma(teacher_xyz_)
            teacher_sigma_before_activation = teacher_sigma[:, 0]
            if list(config.datasetConfDict.values())[0]["dataset"] == "renderingNerfBlender58Pointk8L112V100":
                caseID = config.datasetConfDict["renderingNerfBlender58Pointk8L112V100"]["caseID"]
                if caseID % 8 == 7:  # the ship
                    flags = ((xyz[..., 0] ** 2 + xyz[..., 1] ** 2) > 0.9 ** 2) | (xyz[..., 2] > -0.1)
                else:
                    raise ValueError("Dataset (%s) CaseID (%d) has not yet been supported by the 'needDensityFixer'" % (
                        list(config.datasetConfDict.values)[0], caseID
                    ))
            else:
                raise ValueError("Dataset (%s) has not yet been supported by the 'needDensityFixer'" % list(config.datasetConfDict.values())[0]["dataset"])
            sigma_before_activation[flags] = teacher_sigma_before_activation[flags]
        ...
        if if_compute_normal3:
            ...
            normal3_alpha = F.softplus(self.normal3_alpha(normal3_feat))
            normal3_hdr = None  # now also need to incorporate the point_light_opacity_hint, and shall be predicted in self.appearance(...)
    ...
    def preAppearance(self, x, **kwargs):
        ...
        preAppearancePredictionDict = {
            ...
            "out_normal3_alpha": normal3_alpha,
            "out_normal3_hdr": normal3_hdr,
            "out_normal3_feat": normal3_feat,
        }
    ...
    def appearance(self, x, d, preAppearancePredictionDict, **kwargs):
        ...
        hints_pointlight_opacities = kwargs["hints_pointlight_opacities"]
        # hints_pointlight_ggx = kwargs["hints_pointlight_ggx"]
        ...
        # normal3_hdr
        fullPredictionDict["out_normal3_hdr"] = self.safe_exp(
            self.normal3_hdr(
                torch.cat([
                    preAppearancePredictionDict["out_normal3_feat"],
                    hints_pointlight_opacities_embedded,
                ], 1)
            ),
            1,
