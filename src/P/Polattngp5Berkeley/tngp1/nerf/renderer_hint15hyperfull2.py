...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, models, **kwargs):
        ...
        def sigma_fn_1617(t_starts, t_ends, ray_indices, model1617):
            t_origins = rays_o[ray_indices]
            t_dirs = rays_d[ray_indices]
            positions = t_origins + t_dirs * (t_starts + t_ends)[:, None] / 2.0
            sigmas = model1617.density(
                positions,
        ...
        sigma_fn_16 = partial(sigma_fn_1617, model1617=model16)
        sigma_fn_17 = partial(sigma_fn_1617, model1617=model17)

        render_step_size = get_render_step_size(
            iterCount, self.render_step_size_keys, self.render_step_size_vals)
        ray_indices_17, t_starts_17, t_ends_17 = occGrid.sampling(
            rays_o, rays_d,
            sigma_fn=sigma_fn_17,
        ...
        # avoid empty error
        if ray_indices_17.shape[0] == 0:
            ray_indices_17 = torch.cuda.LongTensor([0], device=rays_o.device)
            t_starts_17 = torch.cuda.FloatTensor([nears[0] if (nears[0] < 1000) else 2], device=rays_o.device)
            t_ends_17 = torch.cuda.FloatTensor([fars[0] if (fars[0] < 1000) else 60], device=rays_o.device)

        # flag_keep (sometimes we need to remove just-surface sampled point for in-scene cameras)
        if if_do_reflection_surface_cutoff:
            flag_keep_17 = self.do_reflection_surface_cutoff(
                ray_indices_17, t_starts_17,
                reflection_surface_cutoff=config.reflection_surface_cutoff,
                render_step_size=render_step_size,               
            )
        else:
            flag_keep_17 = np.ones((ray_indices_17.shape[0],), dtype=bool)
        ray_indices_17 = ray_indices_17[flag_keep_17]
        t_starts_17 = t_starts_17[flag_keep_17]
        t_ends_17 = t_ends_17[flag_keep_17]
        ...
        t_origins_17 = rays_o[ray_indices_17]
        t_dirs_17 = rays_d[ray_indices_17]
        positions_17 = t_origins_17 + t_dirs_17 * (t_starts_17 + t_ends_17)[:, None] / 2.0
        xyzs_17 = positions_17
        dirs_17 = t_dirs_17
        ...
        hyperPredictionDict["insideBatchInd_fgmgbg_point_lgt"] = insideBatchInd_fgmgbg_lgt[ray_indices_17]

        assert self.enable_refnerf
        assert iterCount >= self.empty_cache_stop_iter

        preAppearancePredictionDict_17 = model17.preAppearance(
            xyzs_17,
            if_do_bound_clamp=True,
            requires_grad=False,  # requires_grad,
            iterCount=iterCount,
        )
        # do normal3 predicted by model16
        with torch.no_grad():
            # model17(temp) or model16(should-be)
            # normal3_npp_unormalize_raw_17 = model16.normal3_npp(normal3_feat_17)
            normal3_feat_17 = model17.normal3_encoder(xyzs_17) * 100  # 100: because I am lazy to do tcnn module initialization
            normal3_npp_unormalize_raw_17 = model17.normal3_npp(normal3_feat_17)
            bound_float16 = 6e4  # torch.finfo(torch.float16).max is 65504.0
            normal3_npp_17 = F.normalize(
                torch.clamp(normal3_npp_unormalize_raw_17, min=-bound_float16, max=bound_float16).float(),
                p=2, dim=1
            )
            preAppearancePredictionDict_17["out_normal3_feat"] = normal3_feat_17
            preAppearancePredictionDict_17["out_normal3_npp"] = normal3_npp_17

        sigmas_17 = preAppearancePredictionDict_17["out_tau_spatial"] * self.density_scale

        weights_17, _, _ = render_weight_from_density(
            t_starts_17, t_ends_17, sigmas_17, ray_indices=ray_indices_17, n_rays=int(rays_o.shape[0]),
        )
        opacities_17 = accumulate_along_rays(
            weights_17, values=None, ray_indices=ray_indices_17, n_rays=int(rays_o.shape[0]))[:, 0]
        # depths: this is euclidean depth, rather than zbuffer depth
        depths_17 = accumulate_along_rays(
            weights_17,
            values=(t_starts_17 + t_ends_17)[..., None] / 2.0,
            ray_indices=ray_indices_17,
            n_rays=int(rays_o.shape[0]),
        )[:, 0] / opacities_17.clamp_min(torch.finfo(opacities_17.dtype).eps) 

        # hdr2
        fullPredictionDict = model17.appearance(
            xyzs_17, dirs_17, preAppearancePredictionDict_17,
        ...
        with torch.no_grad():
            if (callFlag != "forwardNetNeRF") and (config.if_highlight_case):
                if config.double_density_net:
                    ray_indices_16, t_starts_16, t_ends_16 = occGrid.sampling(
                        rays_o, rays_d,
                        sigma_fn=sigma_fn_16,
                        near_plane=nears.min(), far_plane=(fars[fars < 1000].max() if torch.any(fars < 1000) else 10 * nears.min()),
                        early_stop_eps=1e-4, alpha_thre=1e-5,
                        render_step_size=render_step_size,
                    )
                    if ray_indices_16.shape[0] == 0:
                        ray_indices_16 = torch.cuda.LongTensor([0], device=rays_o.device)
                        t_starts_16 = torch.cuda.FloatTensor([nears[0] if (nears[0] < 1000) else 2], device=rays_o.device)
                        t_ends_16 = torch.cuda.FloatTensor([fars[0] if (fars[0] < 1000) else 60], device=rays_o.device)
                    # flag_keep (sometimes we need to remove just-surface sampled point for in-scene cameras)
                    if if_do_reflection_surface_cutoff:
                        flag_keep_16 = self.do_reflection_surface_cutoff(
                            ray_indices_16, t_starts_16,
                            reflection_surface_cutoff=config.reflection_surface_cutoff,
                            render_step_size=render_step_size,               
                        )
                    else:
                        flag_keep_16 = np.ones((ray_indices_16.shape[0],), dtype=bool)
                    ray_indices_16 = ray_indices_16[flag_keep_16]
                    t_starts_16 = t_starts_16[flag_keep_16]
                    t_ends_16 = t_ends_16[flag_keep_16]
                    # avoid empty error once again
                    if ray_indices_16.shape[0] == 0:
                        ray_indices_16 = torch.cuda.LongTensor([0], device=rays_o.device)
                        t_starts_16 = torch.cuda.FloatTensor([nears[0] if (nears[0] < 1000) else 2], device=rays_o.device)
                        t_ends_16 = torch.cuda.FloatTensor([fars[0] if (fars[0] < 1000) else 60], device=rays_o.device)
                    t_origins_16 = rays_o[ray_indices_16]
                    t_dirs_16 = rays_d[ray_indices_16]
                    positions_16 = t_origins_16 + t_dirs_16 * (t_starts_16 + t_ends_16)[:, None] / 2.0
                    xyzs_16 = positions_16
                    dirs_16 = t_dirs_16
                    preAppearancePredictionDict_16 = model16.preAppearance(
                        xyzs_16,
                        if_do_bound_clamp=True,
                        requires_grad=False,  # requires_grad,
                        iterCount=iterCount,
                    )
                    normal3_feat_16 = model16.normal3_encoder(xyzs_16) * 100  # 100: because I am lazy to do tcnn module initialization
                    normal3_npp_unormalize_raw_16 = model16.normal3_npp(normal3_feat_16)
                    bound_float16 = 6e4  # torch.finfo(torch.float16).max is 65504.0
                    normal3_npp_16 = F.normalize(
                        torch.clamp(normal3_npp_unormalize_raw_16, min=-bound_float16, max=bound_float16).float(),
                        p=2, dim=1
                    )
                    preAppearancePredictionDict_16["out_normal3_feat"] = normal3_feat_16
                    preAppearancePredictionDict_16["out_normal3_npp"] = normal3_npp_16
                    sigmas_16 = preAppearancePredictionDict_16["out_tau_spatial"] * self.density_scale
                    weights_16, _, _ = render_weight_from_density(
                        t_starts_16, t_ends_16, sigmas_16, ray_indices=ray_indices_16, n_rays=int(rays_o.shape[0]),
                    )
                    opacities_16 = accumulate_along_rays(
                        weights_16, values=None, ray_indices=ray_indices_16, n_rays=int(rays_o.shape[0]))[:, 0]
                    # depths: this is euclidean depth, rather than zbuffer depth
                    depths_16 = accumulate_along_rays(
                        weights_16,
                        values=(t_starts_16 + t_ends_16)[..., None] / 2.0,
                        ray_indices=ray_indices_16,
                        n_rays=int(rays_o.shape[0]),
                    )[:, 0] / opacities_16.clamp_min(torch.finfo(opacities_16.dtype).eps) 
                    normal3_npp = F.normalize(accumulate_along_rays(
                        weights_16,
                        values=preAppearancePredictionDict_16["out_normal3_npp"],
                        ray_indices=ray_indices_16,
                        n_rays=int(rays_o.shape[0]),
                    ), dim=1, p=2)
                    valid_mask = opacities_16 >= 0.99
                    valid_mask[depths_16 < torch.finfo(torch.float32).eps] = False
                    ...
                    ref_point = torch.where(
                        valid_mask_3,
                        rays_o + depths_16[:, None] * rays_d,  # depths is euclidean depth
                        0,
                    ) 
                else:
                    normal3_npp = F.normalize(accumulate_along_rays(
                        weights,
                        values=preAppearancePredictionDict_17["out_normal3_npp"],
                        ray_indices=ray_indices_17,
                        n_rays=int(rays_o.shape[0]),
                    ), dim=1, p=2)
                    valid_mask = opacities_17 >= 0.99
                    valid_mask[depth_17 < torch.finfo(torch.float32).eps] = False
                    valid_mask[torch.norm(normal3_npp, p=2, dim=1) < 0.9] = False
                    ...
                    ref_point = torch.where(
                        valid_mask_3,
                        rays_o + depths_17[:, None] * rays_d,  # depths is euclidean depth
                        0,
                    )
                ...
                hdr3 = self.safe_exp(tmp, 1, 10).float() * opacities_16[:, None]
        ...
        assert len(opacities_17.shape) == 1
        assert len(depths_17.shape) == 1
        results = {}
        ...
        results["depth"] = depths_17
        results["opacity"] = opacities_17
        ...
        # sampling stats
        results["statNumSampledPoints"] = torch.cuda.FloatTensor([int(xyzs_17.shape[0])])
        assert results["statNumSampledPoints"].device == xyzs_17.device
        results["statP2Rratio"] = torch.cuda.FloatTensor([float(xyzs_17.shape[0]) / float(rays_o.shape[0])])
        assert results["statP2Rratio"].device == xyzs_17.device
