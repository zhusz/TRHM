...
class NeRFNetwork(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # self.embeddingHintPointlightGGX = Embedding(config.N_emb_hint_pointlight_ggx)
        ...
        self.normal3_envmap = tcnn.Network(
            # n_input_dims=1 + 3 + in_channels_hint_pointlight_opacities + in_channels_xyz,  # 1 is for envmap (already aggregated), 3 is for reflection hints
            # n_input_dims=1 + 3 + 1,  # ref_hint_env (1) + ref_out["hdr"] (3) + ref_out["opacity"] (1)
            n_input_dims=in_channels_hint_pointlight_opacities + (config.pyramid_len * 1) + 3 + in_channels_xyz,
            # You need to set "1" to be zero if envmap is blocked, or set "3" to be zero if there is no block
            # Soft zero ("1" times the reflection opacity)
            n_output_dims=3,
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
        ...
        self.w_dims_in = [
            # config.sigma_b_dim + 1 + t * in_channels_dir + in_channels_hint_pointlight_opacities,
    ...
    def density(self, xyz, if_do_bound_clamp, if_output_only_sigma, if_compute_normal3, **kwargs):
        # assert self.config.density_fix
        if if_compute_normal3:
    ...
    def preAppearance(self, x, **kwargs):
        ...
        out_tau_spatial, xyz_, normal3_npp, sigma_before_activation, normal3_feat = (
            tmp[0][:, None], tmp[1], tmp[2], tmp[3], tmp[4])
