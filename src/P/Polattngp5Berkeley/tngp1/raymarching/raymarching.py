...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024):
        ...
        if not force_all_rays and mean_count > 0:
            if align > 0:
                num_padded_row = align - mean_count % align
        ...
        return xyzs, dirs, deltas, rays, num_padded_row

march_rays_train = _march_rays_train.apply


class _march_rays_train_raysInfo(Function):  # add teh raysInfo to sample to sampledRaysInfo (e.g. for lgt_omega)
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(
        ctx,
        rays_o,
        rays_d,
        raysInfo,  # the additional raysInfo (e.g. lighting input) to be sampled to the sampled points
        bound,
        density_bitfield,
        C,
        H,
        ...
        rays_o = rays_o.contiguous().view(-1, 3)
        rays_d = rays_d.contiguous().view(-1, 3)
        assert rays_d.shape[0] == rays_o.shape[0]
        DInfo = raysInfo.shape[1]
        assert raysInfo.shape == (rays_o.shape[0], DInfo)
        raysInfo = raysInfo.contiguous().view(-1, DInfo)
        density_bitfield = density_bitfield.contiguous()
        ...
        xyzs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        dirs = torch.zeros(M, 3, dtype=rays_o.dtype, device=rays_o.device)
        sampledRaysInfo = torch.zeros(M, DInfo, dtype=raysInfo.dtype, device=raysInfo.device)
        deltas = torch.zeros(M, 2, dtype=rays_o.dtype, device=rays_o.device)
        rays = torch.empty(N, 3, dtype=torch.int32, device=rays_o.device) # id, offset, num_steps
        ...
        _backend.march_rays_train_raysInfo(
            rays_o,
            rays_d,
            raysInfo,  # the added one
            density_bitfield,
            bound,
            dt_gamma,
            max_steps,
        ...
        if force_all_rays or mean_count <= 0:
            ...
            xyzs = xyzs[:m]
            dirs = dirs[:m]
            sampledRaysInfo = sampledRaysInfo[:m]
            deltas = deltas[:m]

            torch.cuda.empty_cache()

        return xyzs, dirs, sampledRaysInfo, deltas, rays, num_padded_row

march_rays_train_raysInfo = _march_rays_train_raysInfo.apply
...
"""
class _composite_rays_train_refnerf(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, normal_pred, normal_from_tau, deltas, rays, rays_d, T_thresh=1e-4):
        ''' composite rays' rgbs, according to the ray marching formula.
        Args:
            rgbs: float, [M, 3]
            sigmas: float, [M,]
            normal_pred: float, [M, 3]
            normal_from_tau: float, [M, 3]
            deltas: float, [M, 2]
            rays: int32, [N, 3]
            rays_d: float, [N, 3]
        Returns:
            ...
            image: float, [N, 3], the RGB channel (after multiplying alpha!)
            loss_tie: float, [N, ]
            loss_backface: float, [N, ]
        '''
        
        sigmas = sigmas.contiguous()
        rgbs = rgbs.contiguous()
        normal_pred = normal_pred.contiguous()
        normal_from_tau = normal_from_tau.contiguous()
        ...
        depth = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, 3, dtype=sigmas.dtype, device=sigmas.device)
        loss_tie = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        loss_backface = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)

        _backend.composite_rays_train_refnerf_forward(  # 4 3 3 5 (15)
            sigmas, rgbs, normal_pred, normal_from_tau,
            deltas, rays, rays_d,
            M, N, T_thresh,
            weights_sum, depth, image, loss_tie, loss_backface
        )

        ctx.save_for_backward(sigmas, rgbs, normal_pred, normal_from_tau, deltas, rays, rays_d, weights_sum, depth, image)
        ctx.dims = [M, N, T_thresh]

        return weights_sum, depth, image, loss_tie, loss_backface
    
    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights_sum, grad_depth, grad_image, grad_loss_tie, grad_loss_backface):

        # NOTE: grad_depth is not used now! It won't be propagated to sigmas.

        grad_weights_sum = grad_weights_sum.contiguous()
        grad_image = grad_image.contiguous()
        grad_loss_tie = grad_loss_tie.contiguous()
        grad_loss_backface = grad_loss_backface.contiguous()

        sigmas, rgbs, normal_pred, normal_from_tau, deltas, rays, rays_d, weights_sum, depth, image = ctx.saved_tensors
        M, N, T_thresh = ctx.dims
   
        grad_sigmas = torch.zeros_like(sigmas)
        grad_rgbs = torch.zeros_like(rgbs)
        grad_normal_pred = torch.zeros_like(normal_pred)
        grad_normal_from_tau = torch.zeros_like(normal_from_tau)

        _backend.composite_rays_train_refnerf_backward(  # 4 4 5 3 4 (20)
            grad_weights_sum, grad_image, grad_loss_tie, grad_loss_backface,
            sigmas, rgbs, normal_pred, normal_from_tau,
            deltas, rays, rays_d, weights_sum, image,
            M, N, T_thresh,
            grad_sigmas, grad_rgbs, grad_normal_pred, grad_normal_from_tau
        )

        return grad_sigmas, grad_rgbs, grad_normal_pred, grad_normal_from_tau, None, None, None


composite_rays_train_refnerf = _composite_rays_train_refnerf.apply
"""
